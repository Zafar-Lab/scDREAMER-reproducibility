...
def dense(x, inp_dim, out_dim, name = 'dense'):
    ...
    with tf.variable_scope(name, reuse = True): # earlier only tf
        weights = tf.get_variable("weights", shape=[inp_dim, out_dim]
                                  ) # contrib initializer = tf.contrib.layers.xavier_initializer()
...
def dense_(x, inp_dim, out_dim, name = 'dense_'):
    ...
    with tf.variable_scope(name, reuse = None): # earlier only tf
        weights = tf.get_variable("weights_", shape=[inp_dim, out_dim],
                                  initializer = tf.contrib.layers.xavier_initializer()) # contrib
        
        bias = tf.get_variable("bias_", shape=[out_dim], initializer = tf.constant_initializer(0.0))
        
        # initializer= tf2.initializers.GlorotUniform(); same as Xavier's initializer; tf.contrib.layers.xavier_initializer()    
        out = tf.add(tf.matmul(x, weights), bias, name='matmul_')
        return out
