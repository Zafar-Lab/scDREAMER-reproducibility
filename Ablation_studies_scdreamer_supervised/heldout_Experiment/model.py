    ...
    #for it in range(num_batch_iter):
        ...
        if (self.epoch-ep>150):
            for it in range(num_batch_iter):
                ...
                #batch_x, X_ = self.next_batch(self.data_trainS, self.batch_trainS, self.train_sizeS)
                batch_x = self.data_trainS
                X_ = self.batch_trainS
        ...
        #db_loss_curr = 0
        print("Epoch : [%d] ,  a_loss = %.4f, d_loss: %.4f" 
              % (ep, a_loss_curr, d2_loss_curr))
...
class scDREAMER(object):
    def __init__(self, sess, batch, cell_type, name, epoch = 200, lr=0.0007, beta1=0.9, batch_size=128, X_dim=2000, z_dim=10, dataset_name='Pancreas',
        ...
        if self.trans == 'sparse':
            ...
            self.data_train, self.data_trainS, self.data_test, self.scale, self.labels_train, self.labels_trainS, self.labels_test, self.batch_train, self.batch_trainS, self.batch_test, self.batch_info = load_gene_mtx(self.dataset_name, transform=False, count=False, actv=self.gen_activation, batch = self.batch, cell_type = self.cell_type, name = self.name)
        ...
        self.train_sizeS = self.data_trainS.shape[0]
