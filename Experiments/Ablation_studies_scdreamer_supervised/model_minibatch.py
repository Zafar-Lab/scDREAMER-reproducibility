...
import numpy as np
from utils_minibatch import dense,lrelu,zinb_model,eval_cluster_on_test,load_gene_mtx
import pandas as pd
...
def build_model(self):
    ...
    self.decoder_output2 = self.decoder(tf.concat([self.z, self.batch_input],1)) 
    ...
    # cross entropy removed from the loss - tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.disb_real_logit, labels = self.batch_input))
    
    self.autoencoder_loss = - self.ELBO_gauss - tf.log(tf.math.minimum(tf.math.maximum(tf.reduce_sum(tf.sqrt(tf.abs(self.dis2_real_logit/tf.reduce_sum(self.dis2_real_logit)* self.dis2_fake_logit/tf.reduce_sum(self.dis2_fake_logit)))), capL), capU)) - tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = self.disb_real_logit, labels = self.batch_input))              
...
def train_cluster(self):
    ...
    # Discriminator D2
    discriminator2_optimizer = tf.train.AdamOptimizer(learning_rate=self.lr*0.5,
                                                      beta1=self.beta1).minimize(self.dis2_loss,
                                                                                  var_list=self.dis2_vars)
    
    # Discriminator batch: Classifier....
    
    batchClassifier_optimizer = tf.train.AdamOptimizer(learning_rate = self.lr*0.5,
    ...
    # AJ: concatenating x_train and batch info for batches preparation
    X = np.concatenate((np.array(self.data_train), np.array(self.batch_train)), axis = 1)
    # Shuffling of data in each epoch
    ...
    #for it in range(num_batch_iter):
        ...
        index = 0
        np.random.shuffle(X)
        train_data = X[:, :-self.batch_train.shape[1]] # data
        batch_data = X[:, -self.batch_train.shape[1]:] # batch data 
        ...
        for it in range(num_batch_iter):
            ...
            batch_x = train_data[index:(index + self.batch_size), :]
            X_ = batch_data[index:(index + self.batch_size), :]
        ...
        #db_loss_curr = 0
        print("Epoch : [%d] ,  a_loss = %.4f, d_loss: %.4f,  d_loss: %.4f" 
              % (ep, a/num_batch_iter, d2/num_batch_iter, db/num_batch_iter))
        
        
        self._is_train = False # enables false after 1st iterations only...to make training process fast

        if (np.isnan(a_loss_curr) or np.isnan(d_loss_curr)): 
          a_loss_curr = 0
          #d_loss_curr = 0
          #g_loss_curr = 0
          d2_loss_curr = 0
...
def decoder(self, z, reuse=False):
    ...
    with tf.variable_scope('Decoder') as scope:
        ...
        self.n_input = 2000
        self.x_post_scale = tf.nn.softmax(dense(h, self.g_h_dim[0], self.n_input, name='dec_x_post_scale')) 
        self.x_post_r = tf.Variable(tf.random_normal([self.n_input]), name="dec_x_post_r")  

        self.x_post_rate = tf.exp(self.library) * self.x_post_scale
        self.x_post_dropout = dense(h, self.g_h_dim[0], self.n_input, name='dec_x_post_dropout') 

        self.decoder_output2 = tf.nn.sigmoid(dense(h, self.g_h_dim[0], self.X_dim, 'dec_output2'))
            
        return self.decoder_output2 # return h
...
class scDREAMER(object):
    def __init__(self, sess, batch, cell_type, name, epoch = 200, lr=0.0007, beta1=0.9, batch_size=128, X_dim=2000, z_dim=10, dataset_name='Pancreas',
