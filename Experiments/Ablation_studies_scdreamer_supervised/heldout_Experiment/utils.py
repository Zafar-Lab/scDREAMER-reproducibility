...
def prepare_data(Ann, batch, cell_type, name):
...
def read_h5ad_(data_path, batch, cell_type, name, hvg=2000):
    print('updated hvg')
    ...
    Ann_ = Ann.copy()
    Ann = Ann_[Ann_.obs['celltype'] != "delta"]
    subset = Ann_[Ann_.obs['celltype'] == "delta"]
 
    #Ann = Ann_[~Ann_.obs['celltype'].isin(["alpha", "gamma"])]
    #subset = Ann_[Ann_.obs['celltype'].isin(["alpha", "gamma"])]
    
    data, labels, batch_info_enc, batch_info = prepare_data(Ann, batch, cell_type, name)
    dataS, labelsS, batch_info_encS, batch_infoS = prepare_data(subset, batch, cell_type, name)
    
    return data, labels, batch_info_enc, batch_info, dataS, labelsS, batch_info_encS, batch_infoS
...
def load_gene_mtx(dataset_name, name, transform = True, count = True, actv = 'sig', batch = "batch", cell_type = "cell_type"):
    ...
    data_, labels_, batch_info_enc_, batch_info_, dataS, labelsS, batch_info_encS, batch_infoS = read_h5ad_(dataset_name, batch, cell_type, name)
    ...
    labels_  = ord_enc.fit_transform(labels_)
    labelsS  = ord_enc.fit_transform(labelsS)
    #print ('here S', labelsS)
    ...
    # batch_info is passed one time because it is needed only for the plotting. encoded one is needed while training.
    if count == False:
        return data_, dataS, data, scale, labels_, labelsS, labels, batch_info_enc_, batch_info_encS, batch_info_enc, batch_info
