...
def build_model(self):
    ...
    - tf.log(tf.math.minimum(tf.math.maximum(tf.reduce_sum(tf.sqrt(tf.abs(self.dis2_real_logit/tf.reduce_sum\
      (self.dis2_real_logit)* self.dis2_fake_logit/tf.reduce_sum(self.dis2_fake_logit)))), capL), capU))                    
...
def train_cluster(self):
    ...
    discriminatorb_optimizer = tf.train.AdamOptimizer(learning_rate = self.lr_bc,
